...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void bigputc(u16* fb, u16 color, int curs_x, int curs_y, char c, int xsize, int ysize)
{
  int	i, j, k, l, x, y;
  u16	*p;
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void printbig(u16* fb, u16 color, int curs_x, int curs_y, char *s, int xsize, int ysize)
{
    int i;

    for (i = 0 ; *s != NULL ; i ++){
      bigputc(fb, color, curs_x + i*xsize, curs_y, *s++, xsize, ysize);
    }
}
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void putchr(u16* fb, u16 color, int curs_x, int curs_y, char c)
{
  int	i, j;
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void revchar(u16* fb, u16 color, int curs_x, int curs_y, char c)
{
  int	i, j;
  int	x = curs_x*CHAR_WD;
  int	y = curs_y*CHAR_HT;
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void putint_h(u16* fb, u16 color, int curs_x, int curs_y, int num, char c) {
  int	 i, k;
    
  for (i = 0 ; i < c ; i ++) {
    k = num % 16;
    if (k > 9) {
      putchr(fb, color, curs_x - i, curs_y, k + 'A' - 10);
    } else {
      putchr(fb, color, curs_x - i, curs_y, k + '0');
    }
    num = num >> 4;
  }    
}
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void putint_d(u16* fb, u16 color, int curs_x, int curs_y, int i)
{
  char	c;
  int	j;
  ...
  do {
    putchr(fb, color, curs_x--, curs_y, (j % 10) + '0');
    j /= 10;
  } while (j > 0);

  putchr(fb, color, curs_x-1, curs_y, ' ');
  putchr(fb, color, curs_x, curs_y, c); 

  if (i > -10 && i < 10){
    putchr(fb, color, curs_x-2, curs_y, ' '); 
  }
}
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void printstr(u16* fb, u16 color, int curs_x, int curs_y, char *s)
{
    int i;

    for (i = 0 ; *s != NULL; i ++) {
      putchr(fb, color, curs_x + i, curs_y, *s++);
    }
}
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void pset(u16* fb, int x, int y, u16 color)
{
  ((u16 *)fb)[x + SCREEN_WD*y]=color;
}

/*---------------------------------------------------------------------
                  Make a circle on the screen
  ---------------------------------------------------------------------*/
void circle(u16* fb, int x0, int y0, int r, u16 color)
{
  int	x, y, f;
  ...
  while (x >= y) {
    pset(fb, x0 + x, y0 + y, color);
    pset(fb, x0 - x, y0 + y, color);
    pset(fb, x0 + x, y0 - y, color);
    pset(fb, x0 - x, y0 - y, color);
    pset(fb, x0 + y, y0 + x, color);
    pset(fb, x0 - y, y0 + x, color);
    pset(fb, x0 + y, y0 - x, color);
    pset(fb, x0 - y, y0 - x, color);
    if (f >= 0) {
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void line(u16* fb, int x0, int y0, int x1, int y1, u16 color)
{
  int	dx, dy, sx, sy, i, e;
  int	 x = x0;
  int	 y = y0;
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void box(u16* fb, int x0, int y0, int x1, int y1, u16 color)
{
...
/*---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void WriteBG(u16* fb, u8 *gr, u16 *gbuf)
{
  int 	i;
  u16	*p;
